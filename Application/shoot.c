...
extern uint16_t shoot_can_set_current;
...
int16_t shoot_control_loop(void)
{
    ...
    if(shoot_control.shoot_mode == SHOOT_STOP)
    {
    ...
    else if(shoot_control.shoot_mode == SHOOT_ONE){
    	if(num<2000){
    		num++;
    	...
    	if(1000<num&&num<1500){
    		shoot_control.given_current = 400;
    	}
    	else{
    		shoot_control.given_current = 100;
    	}
    }
    else
    {
    ...
    if(shoot_control.shoot_mode != SHOOT_ONE){
...
static void shoot_set_mode(void)
{
    ...
    if(switch_is_down(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL])){
    	shoot_control.shoot_mode = SHOOT_ONE;
    }
